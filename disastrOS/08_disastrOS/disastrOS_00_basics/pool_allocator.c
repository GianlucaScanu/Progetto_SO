...
PoolAllocatorResult PoolAllocator_init(PoolAllocator* a,
  ...
  // now we populate the free list by constructing a linked list  -> Index and int[Index] used like in a slab allocator
...
void* PoolAllocator_getBlock(PoolAllocator* a) {
  if (a->first_idx==-1)  // -1 = NullIdx -> Block not available
    return 0;
  ...
  a->free_list[detached_idx]=DetachedIdx; //DetachedIdx = -2
